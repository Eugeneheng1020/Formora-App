...
/// 确定; ↑↓ and Enter once it has focus; and a standing line that the user can answer in their own words.
struct AskPanel: View {
    ...
    private var index: Int { min(answered.count, questions.count - 1) }
    ...
    private func record(_ answer: AskTool.Answer) {
        let all = answered + [answer]
        if all.count >= questions.count {
            state.askAnswers[conversationID] = nil
            state.chat.answer(conversationID, all)
        } else {
            state.askAnswers[conversationID] = all
        }
    }
}

/// `.choice-option`: the label (12.5/600) and what it means (11, faint); the recommended one says so.
