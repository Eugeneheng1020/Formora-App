...
enum SystemPrompt {
    ...
    static func preamble(_ environment: Environment) -> String {
        ...
        if environment.canAsk {
            rules.append("要用户在几个选项里做决定时，用 ask 工具把选项摆出来，每个选项一句话说明它意味着什么；一次最多 4 个问题。")
        }
        rules += [
